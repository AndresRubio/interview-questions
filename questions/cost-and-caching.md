...
Push these into the same trace as your OpenTelemetry GenAI spans (see [inference-and-systems.md §4](inference-and-systems.md#4-how-do-you-make-a-genai-system-observable-in-production)) so cost attribution and latency/quality observability share one pipeline instead of two.
...
This matters in production for a mundane reason: things that look like caching infrastructure can quietly stop caching without breaking anything visible. Anthropic's own Claude Code changelog has shipped fixes for exactly this shape of bug — for example an entry (v2.1.280, dated 2026-09-22, fetched 2026-09-25) fixing a model switch made mid-task from a host app causing a prompt-cache miss on the very next prompt. Nothing errored; the cache silently stopped paying off. If your dashboard reports a stable hit rate, that's a reason to check harder, not a reason to stop checking — dashboards report what the instrumentation was told to report, not what happened underneath it.
